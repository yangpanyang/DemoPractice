...
"""
    @Time : 2020/11/5 5:55 PM
    @Author : Caroline
    @File : pnn模型实现
    @Description : deep部分将embedding层进行外积/内积操作，再输入到MLP层计算
"""
...
from tensorflow.python.keras.layers import Flatten, Embedding, concatenate, Dense, Dropout, Activation, \
    BatchNormalization, Reshape, Lambda, Add, subtract, dot
...
if __name__ == '__main__':
    ...
    # 构建模型
    # lmbda=0.002, auc=0.74252194
    # lmbda=0.001, auc=0.74179304
    model = pnn_model(sparse_cols, dense_cols, x_train, x_valid, lmbda=0.002)
    model.compile(optimizer="adam",
                  loss="binary_crossentropy",
                  metrics=["binary_crossentropy", tf.keras.metrics.AUC(name='auc')])
    # 回调函数
    file_path = "./models/pnn_model.h5"

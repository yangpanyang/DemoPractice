...
"""
    @Time : 2020/11/20 6:56 PM
    @Author : Caroline
    @File : MMoE模型搭建
    @Description : 注意各个tensor的shape
"""
...
class MMoE(Layer):
    ...
    def __init__(self,
        ...
        self.expert_kernel_constraint = constraints.get(expert_kernel_constraint)  # 参数比较大的时候，可以限制矩阵里面的最大取值
        ...
        # Keras parameter
        self.input_spec = InputSpec(min_ndim=2)  # input的shape是什么样子的
    ...
    def build(self, input_shape):
        ...
        """
        assert input_shape is not None and len(input_shape) >= 2  # 至少是这样的数据 (batch_size, input_dimension)
        ...
        # gate_kernel: [(batch_size, input_dimension, num_experts)] * num_tasks
        ...
        super(MMoE, self).build(input_shape)  # self.build=True，调用父类函数，这个必须写

    # 核心计算逻辑，return的必须是个tensor
    def call(self, inputs, **kwargs):
        ...
        # inputs: (batch_size, input_dimension)
        # f_{i}(x) = activation(W_{i} * x + b), where activation is ReLU according to the paper
        expert_outputs = K.tf.tensordot(a=inputs, b=self.expert_kernels, axes=1)  # (batch_size, units, num_experts)
        ...
        for index, gate_kernel in enumerate(self.gate_kernels):  # num_tasks
            gate_output = K.dot(x=inputs, y=gate_kernel)  # (batch_size, num_experts)，点对点乘法
            # Add the bias term to the gate weights if necessary
            if self.use_gate_bias:
                gate_output = K.bias_add(x=gate_output, bias=self.gate_bias[index])  # (batch_size, num_experts)
            gate_output = self.gate_activation(gate_output)
            gate_outputs.append(gate_output)

        # expert_outputs: (batch_size, units, num_experts)
        # gate_outputs: [(batch_size, num_experts)] * num_tasks
        # f^{k}(x) = sum_{i=1}^{n}(g^{k}(x)_{i} * f_{i}(x))
        for gate_output in gate_outputs:
            expanded_gate_output = K.expand_dims(gate_output, axis=1)  # 扩展维度：(batch_size, 1, num_experts)
            # 填充值：(batch_size, units, num_experts)，相当于把num_experts复制了units倍
            weighted_expert_output = expert_outputs * K.repeat_elements(expanded_gate_output, self.units, axis=1)
            final_outputs.append(K.sum(weighted_expert_output, axis=2))  # (batch_size, units)，逐个expert上的unit进行sum

        return final_outputs  # [(batch_size, units)] * num_tasks

    # 大部分情况会比较智能的推断shape，不能推断的时候在这里处理
